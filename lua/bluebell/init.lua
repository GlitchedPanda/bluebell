...
if is_vim then require "bluebell.lib.vim" end
...
local M = {
	default_options = {
		flavour = "auto",
		background = {
			light = "dark", -- Until I make a light version
			dark = "dark",
		},
		compile_path = vim.fn.stdpath "cache" .. "/bluebell",
...
function M.compile()
	...
	for flavour, _ in pairs(M.flavours) do
		M.flavour = flavour
		require("bluebell.lib." .. (is_vim and "vim." or "") .. "compiler").compile(flavour)
	end
...
local function get_flavour(default)
	...
	if flavour and not M.flavours[flavour] then
		vim.notify(
			string.format(
				"Bluebell (error): Invalid flavour '%s', flavour must be 'dark' or 'auto'",
				flavour
			),
	...
	return flavour or M.options.flavour or vim.g.bluebell_flavour or M.options.background[vim.o.background]
end
...
function M.load(flavour)
	...
	f(flavour or M.options.flavour or vim.g.bluebell_flavour)
end

---@type fun(user_conf: BluebellOptions?)
function M.setup(user_conf)
	...
	if user_conf.auto_integrations == true then
		user_conf.integrations = vim.tbl_deep_extend(
			"force",
			require("bluebell.lib.detect_integrations").create_integrations_table(),
	...
	local hash = require("bluebell.lib.hashing").hash(user_conf)
...
vim.api.nvim_create_user_command(
	"Bluebell",
	function(inp) vim.api.nvim_command("colorscheme bluebell-" .. get_flavour(inp.args)) end,
	{
		nargs = 1,
...
vim.api.nvim_create_user_command("BluebellCompile", function()
	for name, _ in pairs(package.loaded) do
		if name:match "^bluebell." then package.loaded[name] = nil end
	end
	M.compile()
	vim.notify("Bluebell (info): compiled cache!", vim.log.levels.INFO)
	vim.cmd.colorscheme "bluebell"
end, {})

if vim.g.bluebell_debug then
	vim.api.nvim_create_autocmd("BufWritePost", {
		pattern = "*/bluebell/*",
		callback = function()
			vim.schedule(function() vim.cmd "BluebellCompile" end)
		end,
	})
end
