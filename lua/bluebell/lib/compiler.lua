local path_sep = require("bluebell").path_sep
local O = require("bluebell").options
local M = {}
...
function M.compile(flavour)
	local theme = require("bluebell.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[[
...
local h = vim.api.nvim_set_hl]],
	...
	if vim.g.bluebell_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/bluebell_error.lua"
		print(string.format(
			[[Bluebell (error): Most likely some mistake made in your bluebell config
