...
function M.apply(flavour)
	flavour = flavour or require("bluebell").flavour

	local _O, _C, _U = O, C, U -- Borrowing global var (setfenv doesn't work with require)
	O = require("bluebell").options
	C = require("bluebell.palettes").get_palette(flavour)
	U = require "bluebell.utils.colors"
	...
	local theme = {}
	theme.syntax = require("bluebell.groups.syntax").get()
	theme.editor = require("bluebell.groups.editor").get()
	...
	for integration in pairs(O.integrations) do
		...
		if type(O.integrations[integration]) == "table" then
			...
			if O.integrations[integration] == true then
				local default = require("bluebell").default_options.integrations[integration]
		...
		if cot then
			final_integrations = vim.tbl_deep_extend(
				"force",
				final_integrations,
				require("bluebell.groups.integrations." .. integration).get()
			)
		end
	end

	theme.integrations = final_integrations -- plugins
	theme.terminal = require("bluebell.groups.terminal").get() -- terminal colors
